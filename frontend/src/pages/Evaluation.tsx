...
export default function Evaluation() {
  ...
  // derive a couple of handy stats from the report payload
  const derived = useMemo(() => {
    if (!data) {
      return {
        warningTotal: 0,
        infoTotal: 0,
        affectedByBlinding: 0,
        flagsForRow: (flags: string[]) => flags, // identity
      };
    }

    const warningTotal = Number(data.flags_by_severity?.warning ?? 0);
    const infoTotal = Number(data.flags_by_severity?.info ?? 0);

    // number of candidates that had a BLINDING_DELTA flag
    const affectedByBlinding = (data.candidates || []).filter((c: any) =>
      (c.flags || []).includes("BLINDING_DELTA")
    ).length;

    // counts to show in the table chips per mode
    const flagsForRow = (flags: string[]) => {
      if (viewerMode === "recruiter") {
        // only real warnings, hide DEBUG/INFO (we only have types here, so drop DEBUG)
        return flags.filter((t) => t !== "DEBUG");
      }
      if (viewerMode === "ethics") {
        // warnings + info (DEBUG filtered out)
        return flags.filter((t) => t !== "DEBUG");
      }
      // dev => show everything
      return flags;
    ...
    return { warningTotal, infoTotal, affectedByBlinding, flagsForRow };
  }, [data, viewerMode]);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-5xl p-6 space-y-6">
        ...
        {data && (
          <>
            {/* top stats change slightly per mode */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <Stat
                label={viewerMode === "dev" ? "Mean Δ (signed)" : "Mean |Δ|"}
                value={viewerMode === "dev" ? data.mean_delta : data.mean_abs_delta}
              />
              ...
              {viewerMode !== "recruiter" && (
                <Stat label="Candidates affected (blinding)" value={derived.affectedByBlinding} />
              )}
              {viewerMode !== "recruiter" && (
                <Stat label="Flags (warnings)" value={derived.warningTotal} />
              )}
              {viewerMode === "dev" && (
                <Stat label="Flags (info)" value={derived.infoTotal} />
              )}
            </div>
            ...
            {/* per-candidate table (chips filtered by mode) */}
            <div className="rounded-xl bg-white p-4 ring-1 ring-slate-200">
                ...
                <table className="min-w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {data.candidates?.map((c: any) => {
                      const shown = derived.flagsForRow(c.flags || []);
                      // If you later pipe severity down per candidate, you can swap this chip style logic out.
                      return (
                        <tr key={c.candidate_id}>
                          ...
                          <td
                            className={`py-2 pr-4 ${
                              c.delta < 0 ? "text-emerald-700" : c.delta > 0 ? "text-rose-700" : ""
                            }`}
                          >
